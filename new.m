...
clear,clc
global dt Tc;
Tc = 1;dt = 1;N = 1000*Tc/dt;density = 0.850; % 密度初值
T1s =95; % 231
...
for n = 1:N % 离散计算
    ...
    % 出油
    outQual1 = outFuel(time, density);
    outQ1(n) = outQual1;
    outQual2 = outFuel(time+50, density);
    outQ2(n) = outQual2;
    % 减压
        decQ = decFuel(time,density);
        decQ1(n)=decQ;
    % 更新密度
    allM = density*pi*5^2*500;
    density = (allM - outQual1 - outQual2 - decQ + inQual)/(pi*5^2*500);
...
ylim([95 108])
legend('压强曲线','fontsize',8)
xlabel('时间 t(ms)')
ylabel('压强 P(Mpa)')
set(gcf,'units','centimeters') % 标准单位：厘米
set(gcf,'InnerPosition',[0 5 13 2.5]) % 在屏幕的位置，图窗的长宽
% figure
% set(gcf,'units','centimeters') % 标准单位：厘米
...
% figure
% set(gcf,'units','centimeters') % 标准单位：厘米
% set(gcf,'InnerPosition',[5 5 10 3]) % 在屏幕的位置，图窗的长宽
% yyaxis left
...
% set(gcf,'InnerPosition',[5 5 10 3]) % 在屏幕的位置，图窗的长宽
% yyaxis left
% plot((1:N)*dt,outQ1/dt,'b-','Linewidth',0.6)
% % axis([1,Tc*1000,0,4])
% xlabel('时间 t(ms)')
% ylabel('出油 v(mg/ms)')
% yyaxis right
...
% set(gcf,'InnerPosition',[5 5 10 3]) % 在屏幕的位置，图窗的长宽
% yyaxis left
% plot((1:N)*dt,outQ2/dt,'b-','Linewidth',0.6)
% % axis([1,Tc*1000,0,4])
% xlabel('时间 t(ms)')
% ylabel('出油 v(mg/ms)')
% yyaxis right
...
for Tt1 = T1s % 搜索法，进油工作周期
    ...
    for n = 1:N % 离散计算
        ...
        quatyG = quatyG - inQual;
        % 出油
        inQual1 = inFuel(time, density, quatyG, Tt1);
        inQ(n) = inQual1;
        quatyG = quatyG - inQual;
        % 出油
        outQual1 = outFuel(time, density);
...
function decQ = decFuel(time, density)
global dt;
global dt;
C = 0.85; % 流量系数
A = pi*0.7^2;
time = mod(time,130); % 由于周期的存在，时间取周期的模
decQ = dt*C*fA(time)*sqrt(2*(funP2(density)-0.1)/density)*density;
    function y=fA(t)
        if t <50
            y=0;
        elseif t<52
