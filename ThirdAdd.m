...
clear,clc
global dt Tc;
Tc = 5;dt = 1;N = 1000*Tc/dt;density = 0.850; % 密度初值
T1s =130; % 231
...
for n = 1:N % 离散计算
    ...
    density = (allM - outQual1 - outQual2 + inQual)/(pi*5^2*500);
...
set(gcf,'InnerPosition',[5 5 7 2.7]) % 在屏幕的位置，图窗的长宽
% figure
% set(gcf,'units','centimeters') % 标准单位：厘米
...
xlabel('时间 t(ms)')
ylabel('压强差 P(Mpa)')
legend('第一个喷油嘴流速','压强差','fontsize',8)
figure
set(gcf,'units','centimeters') % 标准单位：厘米
set(gcf,'InnerPosition',[5 5 7 2.7]) % 在屏幕的位置，图窗的长宽
yyaxis left
...
xlabel('时间 t(ms)')
ylabel('压强差 P(Mpa)')
legend('第二个喷油嘴流速','压强差','fontsize',8)
...
% hold on
% set(gcf,'units','centimeters') % 标准单位：厘米
% set(gcf,'InnerPosition',[5 5 7.3 3]) % 在屏幕的位置，图窗的长宽
% yyaxis left
% ylim([0,50])
% yyaxis right
% ylim([-5,10])
